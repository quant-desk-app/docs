...
- **`data-ingestion/`** and **`news-ingestion/`**, hub tables, `/api/v2/market-stats`, `/api/v2/candles`, `/api/v2/data-health`.
- Production UI paths no longer use random/mock volume for market stats.
- [Data plane overview](../data/overview).

## X stream (Epic 16)

- **`x-stream/`** — `GET /api/v2/x/feed`, alerts, and an agent context tool.
- [X stream and social alerts](../social/x-stream).

## Docs

- Navigation adds **Data**, **Perp market coverage**, and **X stream** pages.
