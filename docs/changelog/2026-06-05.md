...
- **Per-policy market scope and tighter order controls for agents.** Agent authorizations now accept a per-policy market list and stricter order-management checks, so you can dial in exactly what each agent wallet is allowed to trade. Pairs with [Mikey AI policy controls](/docs/changelog/2026-05-30).
